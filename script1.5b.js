...
function displayQuestion() {
    ...
    questionElement.innerHTML = question.getAttribute('text');
    optionsList.innerHTML = '';
    dataContainer.innerHTML = '';

    // Adiciona a imagem relacionada à questão, se houver
    const dataElements = question.getElementsByTagName('data');
    if (dataElements.length > 0) {
        const imageElement = dataElements[0].getElementsByTagName('image')[0];
        if (imageElement) {
    ...
    for (let i = 0; i < answers.length; i++) {
        ...
        input.name = 'answer';
        input.value = i;
        input.disabled = currentMode === 'reading'; // Desabilita no Modo Leitura
